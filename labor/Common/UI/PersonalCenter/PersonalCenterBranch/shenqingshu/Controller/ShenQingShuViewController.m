...
@interface ShenQingShuViewController ()<UITableViewDelegate,UITableViewDataSource,UINavigationControllerDelegate,UIImagePickerControllerDelegate,FirstShenQingShuTableViewCellDelegate,SecondShenQingShuTableViewCellDelegate>
...
@implementation ShenQingShuViewController
...
- (void)initUI{
    self.titlesLabel.attributedText = [UILabel getABTbody:@"入党申请" font:16 color:SL_UIColorFromHex(333333, 1) zitistyle:@"Source Han Serif CN"];
...
- (void)initmyTableView{
    ...
    [self.myTableView registerNib:[UINib nibWithNibName:@"FirstShenQingShuTableViewCell" bundle:nil] forCellReuseIdentifier:@"FirstShenQingShuTableViewCellID"];
    [self.myTableView registerNib:[UINib nibWithNibName:@"SecondShenQingShuTableViewCell" bundle:nil] forCellReuseIdentifier:@"SecondShenQingShuTableViewCellID"];
    [self.myTableView registerNib:[UINib nibWithNibName:@"ThirdShenQingShuTableViewCell" bundle:nil] forCellReuseIdentifier:@"ThirdShenQingShuTableViewCellID"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        FirstShenQingShuTableViewCell *cell = [self.myTableView dequeueReusableCellWithIdentifier:@"FirstShenQingShuTableViewCellID"];
        cell.delegate = self;
         return cell;
    }else if (indexPath.section == 1){
        SecondShenQingShuTableViewCell *cell = [self.myTableView dequeueReusableCellWithIdentifier:@"SecondShenQingShuTableViewCellID"];
        cell.delegate = self;
         return cell;
    }else{
        ThirdShenQingShuTableViewCell *cell = [self.myTableView dequeueReusableCellWithIdentifier:@"ThirdShenQingShuTableViewCellID"];
        cell.delegate = self;
         return cell;
    }
...
//        NSString *imageBase64 = [imageData base64EncodedString];
    SecondShenQingShuTableViewCell *cell  = [self.myTableView   cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:1]];
    if (self.picTAG == 1) {
        //第一张图片
...
/**提交*/
- (IBAction)submirButtonAction:(id)sender {
    [self requestadminsysjoinusaddJoin];
}
...
//       self.publishButton.backgroundColor = [UIColor colorWithRed:80/255.0 green:220/255.0 blue:142/255.0 alpha:1];
       self.submitButton.userInteractionEnabled = YES;
        NSLog(@"我是url=    %@",result);
        self.url = [NSString stringWithFormat:@"%@%@",VideoHost,result];
...
/**申请入党*/
- (void)requestadminsysjoinusaddJoin{
    NSMutableDictionary *para = [NSMutableDictionary dictionary];
    para[@"body"] = self.content;
    para[@"url"] = self.url;
    [PersonalCenterRequestDatas adminsysjoinusaddJoinrequestDataWithparameters:para success:^(id  _Nonnull result) {
        [self.navigationController popViewControllerAnimated:YES];
