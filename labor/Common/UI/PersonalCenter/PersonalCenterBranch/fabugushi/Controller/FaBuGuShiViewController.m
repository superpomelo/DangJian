...
#import "FourthFaBuGuShiVTableViewCell.h"
#import "PersonalCenterRequestDatas.h"

@interface FaBuGuShiViewController ()<UITableViewDelegate,UITableViewDataSource,UINavigationControllerDelegate,UIImagePickerControllerDelegate,FirstFaBuGuShiVTableViewCellDelegate,FourthFaBuGuShiVTableViewCellDelegate,ThirdthFaBuGuShiVTableViewCellDelegate,SecondFaBuGuShiVTableViewCellDelegate>
@property (weak, nonatomic) IBOutlet UITableView *myTableView;
@property (nonatomic,strong)NSArray *dataArray;
@property (nonatomic,strong)NSMutableArray *coverArrayM;
@property (nonatomic,strong)NSString *content;
@property (nonatomic,strong)NSString *titlet;

@property (weak, nonatomic) IBOutlet UILabel *titlesLabel;
@property (nonatomic,assign)int picTAG;
@end

@implementation FaBuGuShiViewController
...
- (void)initUI{
    self.titlesLabel.attributedText = [UILabel getABTbody:@"发布故事坊" font:16 color:SL_UIColorFromHex(333333, 1) zitistyle:@"Source Han Serif CN"];
}
...
//    self.myTableView.estimatedRowHeight = 70.0;
    [self.myTableView registerNib:[UINib nibWithNibName:@"FirstFaBuGuShiVTableViewCell" bundle:nil] forCellReuseIdentifier:@"FirstFaBuGuShiVTableViewCellID"];
    [self.myTableView registerNib:[UINib nibWithNibName:@"SecondFaBuGuShiVTableViewCell" bundle:nil] forCellReuseIdentifier:@"SecondFaBuGuShiVTableViewCellID"];
    [self.myTableView registerNib:[UINib nibWithNibName:@"ThirdthFaBuGuShiVTableViewCell" bundle:nil] forCellReuseIdentifier:@"ThirdthFaBuGuShiVTableViewCellID"];
    [self.myTableView registerNib:[UINib nibWithNibName:@"FourthFaBuGuShiVTableViewCell" bundle:nil] forCellReuseIdentifier:@"FourthFaBuGuShiVTableViewCellID"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.dataArray.count == 0) {
        ...
        if (indexPath.section == 0) {
            FirstFaBuGuShiVTableViewCell *cell = [self.myTableView dequeueReusableCellWithIdentifier:@"FirstFaBuGuShiVTableViewCellID"];
            cell.delegate = self;
    //        [cell reloadData:self.dataArray[indexPath.row]];
            return cell;
        }else if (indexPath.section == 1){
            SecondFaBuGuShiVTableViewCell *cell = [self.myTableView dequeueReusableCellWithIdentifier:@"SecondFaBuGuShiVTableViewCellID"];
            cell.delegate = self;
    //        [cell reloadData:self.dataArray[indexPath.row]];
            return cell;
        }else if (indexPath.section == 2){
            FourthFaBuGuShiVTableViewCell *cell = [self.myTableView dequeueReusableCellWithIdentifier:@"FourthFaBuGuShiVTableViewCellID"];
    //        [cell reloadData:self.dataArray[indexPath.row]];
            cell.delegate = self;
            return cell;
        }else{
            ThirdthFaBuGuShiVTableViewCell *cell = [self.myTableView dequeueReusableCellWithIdentifier:@"ThirdthFaBuGuShiVTableViewCellID"];
            cell.delegate = self;
    //        [cell reloadData:self.dataArray[indexPath.row]];
            return cell;
        }
...
/**点击图片*/
- (void)FourthFaBuGuShiVTableViewCellButtonActiondelegate:(FourthFaBuGuShiVTableViewCell*)cell button:(UIButton*)sender tag:(int)tag{
    if (tag == 1) {
        self.picTAG = 1;
    }else if (tag == 2) {
        self.picTAG = 2;
...
//        NSString *imageBase64 = [imageData base64EncodedString];
    FourthFaBuGuShiVTableViewCell *cell  = [self.myTableView   cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:2]];
    if (self.picTAG == 1) {
        //第一张图片
...
- (void)requestmyzone{
    ...
    for (int i=0; i<self.coverArrayM.count; i++) {
        if (i==0) {
            [strM appendString:[NSString stringWithFormat:@"%@%@",VideoHost,self.coverArrayM[i]]];
        }else{
            [strM appendString:@","];
            [strM appendString:[NSString stringWithFormat:@"%@%@",VideoHost,self.coverArrayM[i]]];
        }
    ...
    //图片
      para[@"cover"] = strM;
    para[@"title"] = self.titlet;
 
  
    [PersonalCenterRequestDatas registeredupdatemyzonecreatemyzonerequestDataWithparameters:para success:^(id  _Nonnull result) {
