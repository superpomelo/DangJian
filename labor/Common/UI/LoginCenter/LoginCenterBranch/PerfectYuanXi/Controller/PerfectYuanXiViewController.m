...
@interface PerfectYuanXiViewController ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
@property (nonatomic,strong)PerfectYuanXiModel *modelTemp;
@end

@implementation PerfectYuanXiViewController
...
- (void)viewDidLoad {
    ...
    [self initUI];
    [self requestsysdepartmentgetdeptmentbydepdid];
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component{
    PerfectYuanXiModel *models = self.dataArray[row];
    return models.name;
}
//获取用户当前选中的选项
...
/**通过学校id查询系部*/
- (void)requestsysdepartmentgetdeptmentbydepdid{
    NSMutableDictionary *para = [NSMutableDictionary dictionary];
    para[@"deptId"] = self.deptId;
    [LoginRequestDatas sysdepartmentgetdeptmentbydepdidrequestDataWithparameters:para success:^(id  _Nonnull result) {
        self.dataArray = result;
        if (self.dataArray.count>0) {
            PerfectYuanXiModel *models = self.dataArray[0];
