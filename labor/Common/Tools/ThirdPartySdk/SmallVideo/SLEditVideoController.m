...
- (void)saveAlbumBtnClicked:(id)sender {
    ...
    if ([self.delegate respondsToSelector:@selector(SLEditVideoControllerURLActiondelegate:URL:)]) {
        [self.delegate SLEditVideoControllerURLActiondelegate:self URL:self.videoPath];
    }
    [self dismissViewControllerAnimated:NO completion:nil];
...
//        });
////        self.resultBlock(self.videoPath);
//        if (success) {
//            NSLog(@"视频保存至相册 成功");
//        } else {
//            NSLog(@"保存视频到相册 失败 ");
