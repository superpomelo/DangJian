...
- (void)saveAlbumBtnClicked:(id)sender {
    if(self.image) {
        if ([self.delegate respondsToSelector:@selector(SLEditImageControllerimgActiondelegate:img:)]) {
            [self.delegate SLEditImageControllerimgActiondelegate:self img:self.image];
        }
        [self dismissViewControllerAnimated:NO completion:nil];
