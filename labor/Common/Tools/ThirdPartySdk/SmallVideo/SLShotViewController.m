...
@interface SLShotViewController ()<SLAvCaptureToolDelegate,SLEditImageControllerDelegate,SLEditVideoControllerDelegate>
{
...
//    editViewController.resultBlock = ^(UIImage * _Nonnull img) {
//        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//            self.imageresultBlock(img);
//        });
//    };
...
//    self.videoPath = outputFileURL;
    ...
    editViewController.modalPresentationStyle = UIModalPresentationFullScreen;
    editViewController.delegate = self;
//    editViewController.resultBlock = ^(NSURL * _Nonnull url) {
//        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//            self.videoresultBlock(url);
//        });
//    };
...
/**编辑完图片SLEditImageControllerDelegate*/
- (void)SLEditImageControllerimgActiondelegate:(SLEditImageController*)controller img:(UIImage*)img{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if ([self.delegate respondsToSelector:@selector(SLShotViewControllerimgActiondelegate:img:)]) {
            [self.delegate SLShotViewControllerimgActiondelegate:self img:img];
        }
...
- (void)SLEditVideoControllerURLActiondelegate:(SLEditVideoController*)controller URL:(NSURL*)url{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if ([self.delegate respondsToSelector:@selector(SLShotViewControllerVideoActiondelegate:NSURL:)]) {
            [self.delegate SLShotViewControllerVideoActiondelegate:self NSURL:url];
        }
//        [self dismissViewControllerAnimated:NO completion:nil];
    });
}
