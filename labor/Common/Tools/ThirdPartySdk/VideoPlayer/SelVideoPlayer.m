...
- (void)_videoZoomInWithDirection:(UIInterfaceOrientation)orientation
{
    ...
    self.playbackControls.backfullScreenButton.hidden = NO;
}
...
- (void)_videoZoomOut
{
    ...
    self.playbackControls.backfullScreenButton.hidden = YES;
}
...
- (void)videoDidPlayToEnd:(NSNotification *)notify
{
    ...
    if ([self.delegate respondsToSelector:@selector(SelVideoPlayerEnddelegate:)]) {
        [self.delegate SelVideoPlayerEnddelegate:self];
    }
}
