...
- (void)_resetPlaybackControls
{
    ...
    self.backfullScreenButton.hidden = YES;
    self.isShowing = NO;
...
- (void)_showPlaybackControls
{
    ...
    if (_isFullScreen==YES) {
        self.backfullScreenButton.hidden = NO;
    }
...
- (void)setupUI
{
    ...
    [self addSubview:self.backfullScreenButton];
...
- (void)makeConstraints
{
    ...
    NSInteger left = 0;
    if ([IsIphoneX isIphoneX]==YES) {
        left = 40;
       
    }else{
        left = 15;
    }
    [_backfullScreenButton mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (UIButton *)playButton
{
    if (!_playButton){
        ...
        [_playButton setImage:[UIImage imageNamed:@"ic_ldq_play"] forState:UIControlStateNormal];
...
- (UIButton *)backfullScreenButton{
    if (!_backfullScreenButton) {
        _backfullScreenButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [_backfullScreenButton setImage:[UIImage imageNamed:@"ic_return2"] forState:UIControlStateNormal];
        [_backfullScreenButton setImage:[UIImage imageNamed:@"ic_return2"] forState:UIControlStateSelected];
        [_backfullScreenButton addTarget:self action:@selector(fullScreenAction) forControlEvents:UIControlEventTouchUpInside];
    }
    return _backfullScreenButton;
}
/** 全屏切换按钮 */
